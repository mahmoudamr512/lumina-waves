/**
 * Skeleton shown while the contracts list RSC resolves.
 */
export default function Loading() {
  return (
    <section className="space-y-8" aria-busy="true" aria-label="جارٍ تحميل العقود">
      ...
      <div className="overflow-hidden rounded-2xl border border-border-elevation">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="h-16 animate-pulse border-b border-border-elevation bg-surface/40 motion-reduce:animate-none last:border-0"
          />
        ))}
      </div>
    </section>
